...
import { Vote } from "../src/types.ts";

const userName = <string> Deno.args[0];
...
await saveJSON(votes, `${env("DATA")}/${userName}-votes.json`);
await saveJSON(
  votes.filter((v) => v.diffInDays > 2),
  `${env("DATA")}/${userName}-delayed-votes.json`,
);
...
await saveJSON(votersArray, `${env("DATA")}/${userName}-voters.json`);
...
await saveJSON(karma, `${env("DATA")}/${userName}-karma.json`);
